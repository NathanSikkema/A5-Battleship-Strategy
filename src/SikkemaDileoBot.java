...
public class SikkemaDileoBot implements BattleShipBot {
    ...
    private final Point[] directions = {new Point(0, 1),  // right
            new Point(0, -1), // left
            new Point(1, 0),  // down
            new Point(-1, 0)  // up
    };
     ...
     */
    private int[][] buildProbabilityMap(ArrayList<Point> hitList, cellState[][] boardState) {
        int[][] probabilityMap = new int[BattleShip2.BOARD_SIZE][BattleShip2.BOARD_SIZE];

        // Only consider un-sunk ships for probability calculation
        ArrayList<Integer> remainingSizes = new ArrayList<>(6);
        ...
        for (Point hit : hitList)
            for (int dx = -1; dx <= 1; dx++)
                for (int dy = -1; dy <= 1; dy++) {
                    ...
                    int newX = hit.x + dx;
                    int newY = hit.y + dy;
                    if ((newX >= 0 && newX < size && newY >= 0 && newY < size) && boardState[newX][newY] == cellState.UNKNOWN) {
                        probabilityMap[newX][newY] += 20;
                        if (lastHit != null && hitOrientation != orientation.UNKNOWN)
                            if ((hitOrientation == orientation.HORIZONTAL && dx == 0) || (hitOrientation == orientation.VERTICAL && dy == 0))
                                probabilityMap[newX][newY] += 30;
                    }
                }
            ...
            for (int j = 0; j < size; j++)
                if (boardState[i][j] == cellState.MISS)
                    for (Point dir : directions) {
                        int x = i + dir.x;
                        int y = j + dir.y;
                        if (x >= 0 && x < size && y >= 0 && y < size && boardState[x][y] == cellState.UNKNOWN)
                            probabilityMap[x][y] = Math.max(0, probabilityMap[x][y] - 9);
                    }
    ...
    public void fireShot() {
        ...
        if (shot == null && !targetQueue.isEmpty()) {
            int[][] probabilityMap = buildProbabilityMap(hitList, boardState);
            PriorityQueue<Point> maxHeap = new PriorityQueue<>((a, b) -> Integer.compare(probabilityMap[b.x][b.y], probabilityMap[a.x][a.y]));
            for (Point queueShot : targetQueue)
                if (!shotsFired.contains(queueShot) && !uselessLocations.contains(queueShot))
                    maxHeap.offer(queueShot);
            Point bestQueueShot = maxHeap.poll();
            if (bestQueueShot != null) {
                shot = bestQueueShot;
                targetQueue.remove(bestQueueShot);
            }
        }
        if (shot == null) shot = findHighestProbability(buildProbabilityMap(hitList, boardState));
        ...
        if (hit) {
            ...
            if (battleShip.numberOfShipsSunk() > previousSunkShips) {
                ...
                Point[] orderedDirs = {new Point(0, 1),  // right
                        new Point(0, -1), // left
                        new Point(1, 0),  // down
                        new Point(-1, 0)  // up
                };
                ...
                int[][] newProbabilityMap = buildProbabilityMap(hitList, boardState);
